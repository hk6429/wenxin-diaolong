...
export const API_ORIGIN = 'https://wenxin-diaolong.pages.dev';
const SAME_ORIGIN_HOSTS = new Set(['wenxin-diaolong.pages.dev', 'localhost', '127.0.0.1']);
  ...
  return {
    ...
    async call(path, { method = 'POST', body } = {}) {
      if (typeof path !== 'string' || !path.startsWith('/api/')) {
        throw new TypeError(`WXAPI.call path 必須以 /api/ 開頭：${path}`);
      }
      const opts = { method };
      if (body !== undefined) {
...
export const WXAPI = createApi();
if (typeof window !== 'undefined') window.WXAPI = WXAPI;

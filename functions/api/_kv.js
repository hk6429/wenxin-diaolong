...
export function kvFor(db) {
  ...
  return {
    ...
    // 排名（0-based，rev 預設 true＝分數越高名次越前，符合排行榜直覺）；
    // 回傳 { rank, total, score, aheadScore }，aheadScore＝排在自己前一名的分數（供算分差），
    // 若已是第一名則 aheadScore=null。member 不存在回傳 null。
    async zrank(k, member, opts) {
      const rev = !(opts && opts.rev === false);
      const m = typeof member === 'string' ? member : JSON.stringify(member);
      const order = rev ? 'DESC' : 'ASC';
      const { results } = await db.prepare(
        `SELECT member, score FROM zset WHERE k=?1 AND (exp IS NULL OR exp>?2) ORDER BY score ${order}, member ${order}`
      ...
      const rows = results || [];
      const idx = rows.findIndex((r) => r.member === m);
      if (idx < 0) return null;
      return {
        rank: idx,
        total: rows.length,
        score: rows[idx].score,
        aheadScore: idx > 0 ? rows[idx - 1].score : null,
      };
    },

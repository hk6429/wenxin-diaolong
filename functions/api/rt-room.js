...
// POST { op:'push', code, role, token, state }  → 驗證角色憑證後寫入自己的對戰狀態
// POST { op:'poll', code, role, token }         → 驗證角色憑證後讀對方狀態
// —— 非同步挑戰書（不用同時在線）——
...
const newRoleToken = () => randomBytes(24).toString('base64url');
...
const ORIGINS = [
  'https://wenxin-diaolong.vercel.app',
  'https://wenxin-diaolong.pages.dev',
  'https://wenxin-diaolong.netlify.app',
  'http://localhost:8788',
  'http://localhost:8765',
];
const CORS = (request) => ({
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (op === 'create') {
      ...
      const token = newRoleToken();
      await kv.set(keyOf(code), JSON.stringify({ seed, scope: snap.scope }), { ex: TTL });
      await kv.set(`${keyOf(code)}:p1`, JSON.stringify({ snap, state: null, tokenHash: tokenHash(token), hb: Date.now() }), { ex: TTL });
      await kv.set(hbKeyOf(code, 'p1'), String(Date.now()), { ex: TTL });
      return new Response(JSON.stringify({ ok: 1, code, seed, token }), { status: 200, headers });
    }
    ...
    if (op === 'join') {
      ...
      const token = newRoleToken();
      await kv.set(`${keyOf(code)}:p2`, JSON.stringify({ snap, state: null, tokenHash: tokenHash(token), hb: Date.now() }), { ex: TTL });
      await kv.set(hbKeyOf(code, 'p2'), String(Date.now()), { ex: TTL });
      const m = typeof meta === 'string' ? JSON.parse(meta) : meta;
      const o = typeof p1 === 'string' ? JSON.parse(p1) : p1;
      return new Response(JSON.stringify({ ok: 1, seed: m.seed, scope: m.scope, opp: o ? o.snap : null, token }), { status: 200, headers });
    }
    ...
    if (op === 'push') {
      ...
      const obj = cur ? (typeof cur === 'string' ? JSON.parse(cur) : cur) : null;
      if (!obj || !tokenMatches(obj, body.token)) return new Response(JSON.stringify({ error: 'unauthorized role' }), { status: 403, headers });
      if (obj.state && (state.round < obj.state.round || state.dmg < obj.state.dmg || state.correct < obj.state.correct || (obj.state.done && !state.done))) {
        return new Response(JSON.stringify({ error: 'state rollback rejected' }), { status: 409, headers });
      }
      obj.state = state; obj.hb = Date.now();
      await kv.set(`${keyOf(code)}:${role}`, JSON.stringify(obj), { ex: TTL });
      await kv.set(hbKeyOf(code, role), String(obj.hb), { ex: TTL });
    ...
    if (op === 'poll') {
      ...
      const ownRaw = await kv.get(`${keyOf(code)}:${role}`);
      const own = ownRaw ? (typeof ownRaw === 'string' ? JSON.parse(ownRaw) : ownRaw) : null;
      if (!own || !tokenMatches(own, body.token)) return new Response(JSON.stringify({ error: 'unauthorized role' }), { status: 403, headers });
      const other = role === 'p1' ? 'p2' : 'p1';
      const now = Date.now();
      await kv.set(hbKeyOf(code, role), String(now), { ex: TTL });
      const [meta, raw, otherHb] = await Promise.all([
        kv.get(keyOf(code)),
        kv.get(`${keyOf(code)}:${other}`),
        kv.get(hbKeyOf(code, other)),
      ]);
      ...
      return new Response(JSON.stringify({
        ok: 1,
        opp: o ? { snap: o.snap, state: o.state, hb: Number(otherHb) || o.hb } : null,
        now,
      }), { status: 200, headers });
    }
    ...
    // Task 2：你的名次／百分位＋距上一名分差——不在前 10 名也想知道自己排哪。
    if (op === 'seasonRank') {
      const { nick } = body;
      if (!okNick(nick)) return new Response(JSON.stringify({ error: 'bad req' }), { status: 400, headers });
      const season = currentSeasonKey();
      const info = await kv.zrank(seasonKeyOf(season), stripBad(nick).trim().slice(0, 12), { rev: true });
      if (!info) return new Response(JSON.stringify({ ok: 1, season, ranked: false }), { status: 200, headers });
      const gapToNext = info.aheadScore == null ? 0 : Math.max(0, info.aheadScore - info.score);
      const percentile = info.total > 0 ? Math.round(((info.total - info.rank) / info.total) * 100) : 100;
      return new Response(JSON.stringify({
        ok: 1, season, ranked: true,
        rank: info.rank + 1, // 對外一律用 1-based 名次，符合「排名第 X」的直覺
        total: info.total,
        score: info.score,
        percentile, // 贏過幾 % 的人（100=第一名）
        gapToNext, // 距上一名還差幾分；已是第一名則 0
      }), { status: 200, headers });
    }

...
if (!(await page.isVisible('#btn-report'))) fail('全站沒有可見的問題回報入口');
await page.click('#btn-report');
await page.waitForSelector('#report-overlay:not([hidden])');
if (await page.locator('#report-form input[name="reason"]').count() !== 5) fail('問題回報未提供五種快速分類');
if (!(await page.textContent('#report-form'))?.includes('不需填姓名、班級或聯絡資料')) fail('問題回報沒有個資提醒');
await page.click('#btn-report-cancel');
await page.waitForSelector('#report-overlay[hidden]', { state: 'attached' });
...
await page.route('**/api/report', async (route) => {
  ...
  await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true, id: 'smoke-report' }) });
});
await page.click('#btn-report');
await page.click('#btn-report-submit');
await page.waitForFunction(() => document.querySelector('#report-status')?.textContent.includes('已送出'));
if (reportPayload?.reason !== 'progress') fail('問題回報未保留預設的進度異常分類');
if (reportPayload?.context?.screen !== 'screen-practice' || reportPayload?.context?.level !== '國小') fail('問題回報未自動附上畫面與學段');
if (!reportPayload?.context?.question?.id || !reportPayload.context.question.prompt) fail('問題回報未自動附上當下題目');
await page.click('#btn-report-cancel');
